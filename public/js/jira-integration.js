 ...
 * Fixed version with proper inline assessment integration
 */
...
window.JIRAIntegration = {
    ...
    init: function() {
        ...
        // Add buttons to inline assessment system
        this.addButtonsToInlineAssessment();
        
        // Add floating button as fallback
        this.addFloatingJIRAButton();
    ...
    /**
     * Add JIRA buttons to inline assessment completion section
     */
    addButtonsToInlineAssessment: function() {
        // Try multiple times to catch the assessment overlay when it appears
        const attempts = [100, 500, 1000, 1500, 2000, 3000];
        
        attempts.forEach(delay => {
            setTimeout(() => {
                this.insertJIRAButtons();
            }, delay);
        });
    },

    /**
     * Insert JIRA buttons into the assessment UI
     */
    insertJIRAButtons: function() {
        // 1. Try to add to inline completion actions (the button container after assessment completes)
        const completionActions = document.getElementById('inlineCompletionActions');
        if (completionActions) {
            const textCenter = completionActions.querySelector('.text-center');
            if (textCenter && !document.getElementById('jiraBtnInCompletion')) {
                const jiraBtn = this.createJIRAButton('jiraBtnInCompletion');
                textCenter.appendChild(jiraBtn);
                console.log('✅ Added JIRA button to completion actions');
            }
        }
        
        // 2. Try to add to results content (when viewing results)
        const resultsContent = document.getElementById('inlineResultsContent');
        if (resultsContent) {
            const actionButtons = resultsContent.querySelector('.row.mb-4'); // The button row in results
            if (actionButtons && !document.getElementById('jiraBtnInResults')) {
                // Add as another column in the button row
                const colDiv = document.createElement('div');
                colDiv.className = 'col-md-3 mb-3';
                colDiv.id = 'jiraBtnInResults';
                
                const jiraBtn = this.createJIRAButton('jiraBtnResults');
                jiraBtn.className = 'btn btn-warning btn-custom w-100';
                jiraBtn.style.cssText = 'background: #0052CC !important; border-color: #0052CC !important;';
                
                colDiv.appendChild(jiraBtn);
                actionButtons.appendChild(colDiv);
                console.log('✅ Added JIRA button to results section');
            }
        }
    },

    /**
     * Create a JIRA button element
     */
    createJIRAButton: function(id) {
        ...
        btn.id = id;
        btn.innerHTML = '<i class="fab fa-jira me-2"></i>Create JIRA Tickets';
        btn.style.cssText = `
        ...
        `;
        btn.onclick = () => this.createTicketsNow();
        btn.title = 'Create JIRA tickets for compliance gaps found in your assessment';
        
        return btn;
    },

    /**
     * Add floating JIRA button as fallback/always-visible option
     */
    addFloatingJIRAButton: function() {
        // Remove existing floating button
        const existingBtn = document.getElementById('floatingJIRABtn');
        if (existingBtn) existingBtn.remove();

        const jiraBtn = document.createElement('button');
        jiraBtn.id = 'floatingJIRABtn';
        jiraBtn.innerHTML = '<i class="fab fa-jira" style="font-size: 20px;"></i><span style="margin-left: 8px;">JIRA</span>';
        ...
        // Add hover effect
        jiraBtn.onmouseenter = function() {
            this.style.transform = 'scale(1.05)';
            this.style.boxShadow = '0 6px 20px rgba(0,82,204,0.5)';
        };
        jiraBtn.onmouseleave = function() {
            this.style.transform = 'scale(1)';
            this.style.boxShadow = '0 4px 15px rgba(0,82,204,0.4)';
        };
        
        jiraBtn.onclick = () => this.createTicketsNow();

        document.body.appendChild(jiraBtn);
        console.log('✅ Added floating JIRA button (always visible)');
        
        // Show notification that JIRA is ready
        setTimeout(() => {
            this.showNotification('JIRA Integration ready! Click the JIRA button to create tickets for compliance gaps.', 'success');
        }, 2000);
    },
    ...
    createTicketsNow: function() {
        ...
        if (!framework) {
            alert('Please complete an assessment first to identify compliance gaps.\n\nClick "Start Assessment" on any framework, answer all questions, then click "Create JIRA Tickets".');
            return;
        }
        
        const gaps = this.identifyGaps(framework);
        if (gaps.length === 0) {
            alert('No compliance gaps found! All controls are 100% compliant.\n\nGreat job on your compliance assessment!');
            return;
        }
        
        // Show confirmation
        const confirmMsg = `Found ${gaps.length} compliance gaps in your ${framework} assessment.\n\nThis will create ${gaps.length} JIRA tickets in project "${this.config.projectKey}".\n\nDo you want to continue?`;
